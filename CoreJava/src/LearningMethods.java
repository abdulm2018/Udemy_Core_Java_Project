
public class LearningMethods {
	
	int i;// delation, intialize 
	int j;
	//display 
	
	//Return type --> data Type of the value returned by method
	
	// Keyword --> return
	
	public void show()
	{
		int a;
		int b;
		String c;
		
	}
	
	
	public void display(int k,String a, char c, float f, boolean b) // declare --> define (If you don't want to return anything put Void as a return type)
	{
		
		//Starts
		/*
		 * 
		 * 
		 * all definitions goes here !!!
		 * 
		 * 
		 */
		
		//int j = 100;
		System.out.println("Inside Display Method");
		//String x = "Hello";
		//return x; // last statement
		
		
		
	}
	
	

	public static void main(String[] args) {
		
		
//		LearningMethods learn = new LearningMethods();
//		//l.display(k, a, c, f, b);
//		learn.show();
//		
//		LearningMethods learn1 = new LearningMethods();
//		learn1.show();
//		
//		Home h = new Home(); // h --> Object --> i = 10
//		h.i++; // Object1 --> i = 11;
//		
//		Home h1 = new Home(); // h1 --> Object2 --> i =10
//		h1.i++; // Object2 --> i = 11;
//		
//		Home h2 = new Home(); // h2 --> Object3 --> i =1
//		System.out.println(h2.i);
//		
		
		calculator calc = new calculator();
		int i = calc.getSum(5, 4); // Method Arguments
		System.out.println("The sum is : "+i);
		
		// Different way to get result
		
		System.out.println(calc.getMult(10,4));
		
		System.out.println(calc.getSub(10,4));
		
		System.out.println(calc.getDiv(10,4));
		
		
		
	

	}

}
